...
const char *			CL_FILE_NAME = { "MultiplyReduction.cl" };
...
{
	csv.open("MultiplyReduction.csv", ofstream::app);
	...
	size_t dataSize2 = NUM_WORK_GROUPS * sizeof(float);
	...
	cl_mem dC = clCreateBuffer( context, CL_MEM_WRITE_ONLY, dataSize2, NULL, &status );
	if( status != CL_SUCCESS )
	...
	status = clEnqueueReadBuffer( cmdQueue, dC, CL_TRUE, 0, dataSize2, hC, 0, NULL, NULL );
	if( status != CL_SUCCESS )
	...
	double perform = (double)NUM_ELEMENTS/(time1-time0)/1000000000.;
	csv << perform;
	csv << ",";
	csv.close();
#ifdef WIN32
